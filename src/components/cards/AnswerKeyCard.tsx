...
import { Key, Calendar, ExternalLink, Clock } from 'lucide-react';
...
export const AnswerKeyCard: React.FC<AnswerKeyCardProps> = ({ item }) => {
  return (
    <div className="rounded-xl p-5 bg-slate-900/60 hover:bg-slate-900/90 border border-slate-800/80 hover:border-slate-700/80 transition-all flex flex-col justify-between group">
        ...
        <div className="space-y-1.5 text-xs text-slate-300 bg-slate-950/40 p-3 rounded-lg border border-slate-800/50 mb-4">
          ...
          {item.objectionLastDate && (
            <div className="flex justify-between items-center">
              ...
              <span className="text-amber-400 font-medium">Until {item.objectionLastDate}</span>
            </div>
          )}
        </div>
      </div>
      ...
      <div className="pt-2 border-t border-slate-800/60 flex items-center justify-between gap-2">
        <span className="text-[11px] text-slate-400">Question Paper & Keys</span>
        <a
          href={item.viewUrl}
          target="_blank"
